...
export default function CreateListing() {
    ...
    const handleImageSubmit = () => {
        ...
        if(files.length + formData.imageUrls.length <1) {
            setImageUploadError("Atlest select one image");
            return;
        }

        if(files.length + formData.imageUrls.length >7) {
            setImageUploadError("You can have max. 6 images Per Listing");
            return;
        }
    ...
    const handleSubmit = async (e)=> {
       e.preventDefault();
       try{
        if(formData.imageUrls.length <1 ) return setError("You must have at least one image");
  ...
  return (
    <main className='p-3 max-w-4xl mx-auto'>
        <h1 className='text-3xl text-center font-semibold my-7'>Update Your Listing</h1>
        <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
            <div className='flex flex-col gap-4 flex-1'>
                ...
                <div className='flex flex-wrap gap-6'>
                    <div className='flex items-center gap-2'>
                        <input onChange={handleChange} value={formData.bedrooms} type="number" id='bedrooms' min='1' required className='p-3 border border-grey-300 rounded-lg'/>
                        <p>Beds</p>
                    </div>
                    <div className='flex items-center gap-2'>
                        <input onChange={handleChange} value={formData.bathrooms} type="number" id='bathrooms' min='1' required className='p-3 border border-grey-300 rounded-lg'/>
                        <p>Baths</p>
                    </div>
                    <div className='flex items-center gap-2'>
                    ...
                    {formData.offer && (
                        <div className='flex items-center gap-2'>
                        <input onChange={handleChange} value={formData.discountPrice} type="number" id='discountPrice' min='1' required className='p-3 border border-grey-300 rounded-lg'/>
                        <div className="flex flex-col items-center">
                            <p>Discount price</p>
                            <span className='text-xs'>($ / month)</span>
                        </div>
            ...
            <div className="flex flex-col flex-1 gap-4">
                ...
                <div className="flex gap-4">
                    <input onChange={(e)=>setFiles(e.target.files)} type="file" id='images' accept='images/*' multiple className='p-3 border border-gray-300 rounded'/>
                ...
                <p className="text-red-700 text-sm">{imageUploadError && imageUploadError}</p>
                {
                    formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => (
                        <div key={url} className="flex justify-between p-3 border items-center">
                            <img src={url} alt="listing name" className="w-20 h-20 object-cover rounded-lg"/>
